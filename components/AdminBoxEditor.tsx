...
import { InteriorBox, BoxFaceTextures } from '../types';
import { ref, uploadBytesResumable, getDownloadURL } from 'firebase/storage';
import { storage } from '../firebase';
import { v4 as uuidv4 } from 'uuid';
import { Upload, Loader2, Trash2, Image as ImageIcon, X } from 'lucide-react';

interface AdminBoxEditorProps {
  box: InteriorBox;
  onSave: (box: InteriorBox) => void;
  onDelete: (id: string) => void;
  onCancel: () => void;
}

const FACE_NAMES: { key: keyof BoxFaceTextures; label: string; indicator: string }[] = [
  { key: 'front',  label: 'Front',  indicator: '▣' },
  { key: 'back',   label: 'Back',   indicator: '▣' },
  { key: 'left',   label: 'Left',   indicator: '◧' },
  { key: 'right',  label: 'Right',  indicator: '◨' },
  { key: 'top',    label: 'Top',    indicator: '⬒' },
  { key: 'bottom', label: 'Bottom', indicator: '⬓' },
];

const AdminBoxEditor: React.FC<AdminBoxEditorProps> = ({ box, onSave, onDelete, onCancel }) => {
  const [formData, setFormData] = useState<InteriorBox>({ ...box });
  const [uploadingFace, setUploadingFace] = useState<string | null>(null);
  const [uploadProgress, setUploadProgress] = useState(0);

  // Keep formData in sync when the box prop changes externally
  ...
  const handleUniformUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    const fileId = uuidv4();
    const path = `interior_boxes/${formData.id}/uniform_${fileId}_${file.name}`;
  ...
  // ---- Per-face upload ----
  const handleFaceUpload = (faceKey: keyof BoxFaceTextures, e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;
    const fileId = uuidv4();
    const path = `interior_boxes/${formData.id}/${faceKey}_${fileId}_${file.name}`;
    handleUpload(
      file,
      path,
      (url) => {
  ...
  return (
    <div className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-[110] bg-[#0a0a0a]/90 backdrop-blur-3xl border border-[#005e99]/40 w-[420px] max-h-[80vh] flex flex-col rounded-2xl shadow-2xl animate-in zoom-in-95 duration-300">
      ...
      <div className="flex justify-between items-center p-6 pb-3 shrink-0">
        <div>
          <h3 className="font-serif text-xl text-white tracking-tight">Edit Box</h3>
